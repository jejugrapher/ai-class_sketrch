...
function drawBadge(sp) {
  ...
  var inScene = !!(sp.ctrl && !sp.ctrl.follow);      // 친구·씨름·춤 중에는 계속 보여 준다
...
function step(now) {
  ...
  sprites.slice().sort(function (p, q) { return p.depth - q.depth; }).forEach(function (sp) { drawSprite(sp, t); drawBadge(sp); });
